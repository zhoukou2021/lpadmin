...
<html lang="zh-cn">
    ...
    <body class="pear-container">
        ...
        <script>
            ...
            const SELECT_API = "{{ route('lpadmin.admin.index') }}";
            const UPDATE_API = "{{ route('lpadmin.admin.update', ':id') }}";
            const DELETE_API = "{{ route('lpadmin.admin.destroy', ':id') }}";
            const STATUS_API = "{{ route('lpadmin.admin.toggle_status', ':id') }}";
            const INSERT_URL = "{{ route('lpadmin.admin.create') }}";
            const UPDATE_URL = "{{ route('lpadmin.admin.edit', ':id') }}";
            ...
            layui.use(["table", "form", "common", "popup", "util"], function() {
				...
				let cols = [
					{
						...
						field: "avatar",
						templet: function (d) {
							return '<img src="'+(d['avatar'] || '/static/admin/images/avatar.png')+'" style="max-width:32px;max-height:32px;" alt="" />'
						},
                        width: 90,
					},{
						title: "邮箱",
						...
						templet: function (d) {
							...
							let items = [];
							if (d[field] && d[field].length > 0) {
								layui.each(d[field], function (k , v) {
									items.push(v.display_name || v.name);
								});
							}
                        ...
                        templet: function (d) {
                            ...
                            form.on("switch("+field+")", function (data) {
                                ...
                                $.post(url, postData, function (res) {
                                    layer.close(load);
                                    if (res.code !== 0) {
                                        return layui.popup.failure(res.message, function () {
				...
				// 获取表格中下拉或树形组件数据
				let apis = [];
				apis.push(["roles", "{{ route('lpadmin.role.index') }}?format=select"]);
				let apiResults = {};
				apiResults["roles"] = [];
                ...
                let doRemove = function (ids) {
                    layer.confirm("确定删除?", {
                        ...
                        if (Array.isArray(ids)) {
                            // 批量删除
                            $.ajax({
                                url: "{{ route('lpadmin.admin.batch_delete') }}",
                                data: {
                                    ids: ids,
